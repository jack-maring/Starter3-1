...
public class SockServer {
  ...
  static JSONObject charCount(JSONObject req) {
    ...
    JSONObject test1 = testField(req, "findchar");
    if (!test1.getBoolean("ok")) {
      return test1;
    }

    JSONObject test2 = testField(req, "count");
    if (!test2.getBoolean("ok")) {
      return test2;
    }

    boolean findChar = req.getBoolean("findchar");
    String string = req.getString("count");
    ...
    if (findChar) {
      String targetChar = req.getString("find");
      int targetCount = 0;

      for (int i=0; i<string.length(); i++) {
        if (string.charAt(i) == targetChar.charAt(0)) {
          targetCount++;
        }
      }
      res.put("result", targetCount);
    } else {
      res.put("result", string.length());
    }
  ...
  static JSONObject inventory(JSONObject req) {
    ...
    JSONObject test1 = testField(req, "task");
    if (!test1.getBoolean("ok")) {
      return test1;
    }
    ...
    if (task.equals("view")) {
      ...
      if (inventory.containsKey(product) && inventory.get(product) >= quantity) {
        ...
        res.put("ok", false);
        res.put("message", "That quantity of " + product + " is not available.");
        return res;
      }

    } else {
      res.put("ok", false);
      res.put("message", "Improper task entered");
      return res;
    }

    res.put("inventory", inventory.toString());
    return res;
  }
